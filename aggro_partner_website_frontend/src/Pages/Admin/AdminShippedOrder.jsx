...
import { changeOrderStatusForAdmin, getAdminShippedOrders } from '../../Services/AdminService';
import { Box, Button, Card, Step, StepLabel, Stepper } from '@mui/material';
import CallIcon from '@mui/icons-material/Call';
import LocationOnIcon from '@mui/icons-material/LocationOn';
import { toast } from 'react-toastify';

const AdminShippedOrder = () => {

    const [placeOrderData, setPlaceOrderData] = useState([]);

    useEffect(() => {
        getAdminShippedOrders().then((response) => {
            setPlaceOrderData(response.data);
        })
    })

    const handleOutForDeliveryOrder=(mainId)=>{
        const orderStatus = "outForDelivery";

        changeOrderStatusForAdmin(mainId,orderStatus).then((response)=>{
            toast.success("Order Out For Delivery Successfull....")
        }).catch((error)=>{
            toast.error("Order Out For Delivery Faild ...!!");
        })
    }
  ...
  return (
    <div className='flex flex-col items-center'>
            ...
            <div className='h-full w-[90rem] bg-gray-100 mt-[5rem] flex flex-col items-center '>
                ...
                <h2 className='text-2xl font-bold text-center py-6'>Shipped Order's</h2>
                    ...
                    {placeOrderData.map((item, num) =>
                        ...
                        <Card className='p-5 w-[80rem] h-[40rem] item-center justify-between box  mt-6 cursor-pointer mb-[3rem]'>
                            ...
                            <div className='flex flex-col items-center mt-10'>
                                <Button sx={{ py: 1.5, mt: 2, bgcolor: "rgb(145 85 253)" }} size='large' variant='contained' onClick={()=> handleOutForDeliveryOrder(item.id)}>Out For Delivery</Button>
                            </div>
